...
* Mike Heard: presenting Joe's proposed five basic tenents for UDP options. I agree with all except E. Incomplete frameworks could lead to issues. Worthwhile to ask the WF to reply to Joe's message and see if we're on the right track.
...
* Mike: Fragmentation specification is not coherent on these topics, so this needs to be checked, we do not seem to have much feedback on this topic. This is central and needs to be right. We do not know what the working group thinks on Encryption and Authentication, people have commented on the maturity on the list.
* Chairs: We need more review on these three topics before the document can move forward.
...
Nicolas Kuhn, Stephan Emile, Gorry Fairhurst, Christian Huitema
    ...
    * Martin: There is more work: The cwnd resumption is communication from the sender and communicate to itself. The naive way is to cache the BDP at some random time on the sawtooth, but this is onlty a part of the problem. When do you cache the cwnd?
        * Gorry: That's an Elephant in the Room I was avoiding, because it is work to come: Our draft talks about this as the observation phase, this needs more work, please help.
    ...
    * Michael Tüxen: How long in the past can this information remain valid?
    ...
    Flow Control rwnd/flow credit is set by the receiver, and would constrain the benfit, unless we update this.
    ...
    * Matt: The problem (earlier) of how to decay information has been an open issue. I'd like to encourage someone to do resaerch into ML strategiesfor this. It should be learned for a connection's own traffic.
        ...
        * Gorry: I agree, more work needed, please help decide what we should write.
...
IPR declarations for various options.
   ...
   * Hannes : Some industrial IoT applications are also long-lived. I am suspcious whether the requirements came first or the solution?
    ...
    * Hannes : What does the stack availability refer to ? Is there an off-the-shelf stack for the DTLS features presented? WolfSSL has a TLS 1.3 and DTLS 1.3 stack... There is a 1.2 stack in MbedTLS.
...
* Chairs : Please read the document and comment on list. The chairs think this is in a good position for a new revision, and we plan to start a WGLC before the next IETF meeting.
...
* Chairs: The purpose of a WGLC is to gather review and indicate support or not, and we therefore plan to start a  WGLC before the next IETF meeting and report on this at TSVWG for IETF-118.
